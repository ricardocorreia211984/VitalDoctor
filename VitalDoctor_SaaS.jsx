    ...
    <div className="fade">
      ...
      <div className="card">
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
          {[
            ["🩺","Nova Consulta","metodo","consulta"],
            ["🗺️","Mapeamento","metodo","mapeamento"],
            ["👥","Pacientes","pacientes",null],
            ["📅","Agenda","agenda",null],
            ["📋","Questionarios","metodo","questionario"],
            ["🌿","Farmacia","metodo","farmacia"],
      ...
      <div className="fade">
        ...
        <div className="card">
          ...
          {passo.id==="acolhimento" && (
            <textarea className="inp" rows={3} placeholder="Notas de acolhimento (opcional)..." value={notas.acolhimento||""} onChange={e=>setNotas(n=>({...n,acolhimento:e.target.value}))} />
          )}
          ...
          {passo.id==="devolutiva" && <>
            ...
            {devolutivaText && <button className="btn btn-s btn-sm" style={{width:"auto",marginTop:4}} onClick={()=>navigator.clipboard?.writeText(devolutivaText)}>📋 Copiar</button>}
          </>}
          ...
          {passo.id==="protocolo" && <>
            ...
            <div className="lbl">Componentes do protocolo</div>
            {PROTOCOLO.componentes.map((c,ci)=>(
              <label key={ci} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,cursor:"pointer",fontSize:11,color:"#b0c4d8"}}>
                <input type="checkbox" checked={protocolChecks.includes(c)} onChange={ev=>{if(ev.target.checked)setProtocolChecks(p=>[...p,c]);else setProtocolChecks(p=>p.filter(x=>x!==c));}} />
                {c}
              </label>
            ))}
            <textarea className="inp" rows={3} placeholder="Notas adicionais do protocolo (alimentos, práticas, áudios...)..." value={notas.protocolo||""} onChange={e=>setNotas(n=>({...n,protocolo:e.target.value}))} style={{marginTop:6}} />
          </>}
        </div>
...
function NovaConsulta({ user, onIniciar }) {
  ...
  const tipoObj = TIPOS_ATENDIMENTO.find(x => x.id === tipoSel);
  const podeIniciar = !!tipoObj && (!tipoObj.subconsultas || !!subSel);
  ...
  return (
    <div className="fade">
      ...
      <div className="card">
        <div className="card-t">Escolhe o tipo de consulta</div>
        <div className="al al-i" style={{ fontSize: 10, marginBottom: 10 }}>
          Selecciona o tipo — a app mostra o passo a passo completo para te guiar durante o atendimento.
        </div>
        {TIPOS_ATENDIMENTO.map(x => (
          <div key={x.id}
            onClick={() => { setTipoSel(x.id); setSubSel(null); }}
            style={{
              cursor: "pointer", padding: "12px 14px", marginBottom: 7, borderRadius: 9,
              border: `2px solid ${tipoSel === x.id ? "#00c6b8" : "#0d1828"}`,
              background: tipoSel === x.id ? "#0d2535" : "#050810", transition: "all .15s"
            }}>
            <div style={{ fontWeight: 700, fontSize: 12, color: "#b0c4d8" }}>{x.nome}</div>
            <div style={{ fontSize: 10, color: "#5a7a9a", marginTop: 3, lineHeight: 1.5 }}>{x.indicado}</div>
            {x.nota && <div style={{ fontSize: 9, color: "#2d4a66", marginTop: 2 }}>{x.nota}</div>}
          </div>
        ))}
      </div>
      ...
      {tipoObj?.subconsultas && (
        <div className="card">
          <div className="card-t">Qual sessão do pack?</div>
          {tipoObj.subconsultas.map(sc => (
            <div key={sc.id}
              onClick={() => setSubSel(sc.id)}
              style={{
                cursor: "pointer", padding: "10px 13px", marginBottom: 6, borderRadius: 8,
                border: `2px solid ${subSel === sc.id ? "#00c6b8" : "#0d1828"}`,
                background: subSel === sc.id ? "#0d2535" : "#050810"
              }}>
              <div style={{ fontWeight: 600, fontSize: 11, color: "#b0c4d8" }}>{sc.nome}</div>
              {sc.nota && <div style={{ fontSize: 9, color: "#5a7a9a", marginTop: 2 }}>{sc.nota}</div>}
            </div>
      ...
      {tipoObj && (
        <div className="card">
          <div className="card-t">
            📋 Passos desta consulta
            {passosDoTipo(tipoSel, subSel).some(p => p.id === "mapeamento")
              ? <span style={{ color: "#f59e0b", marginLeft: 8, fontSize: 8 }}>· inclui mapeamento corporal</span>
              : <span style={{ color: "#2d4a66", marginLeft: 8, fontSize: 8 }}>· sem mapeamento corporal</span>}
          </div>
          <ol style={{ margin: "0 0 14px 16px", padding: 0, fontSize: 11, color: "#5a7a9a" }}>
            {passosDoTipo(tipoSel, subSel).map((p, i) => (
              <li key={p.id} style={{ marginBottom: 6 }}>
                <strong style={{ color: "#b0c4d8" }}>{p.titulo}</strong>
                {p.descricao ? <span style={{ color: "#3d5a7a" }}> — {p.descricao}</span> : ""}
              </li>
            ))}
          </ol>
          <button
            ...
            onClick={() => podeIniciar && onIniciar && onIniciar(tipoSel, subSel)}
            style={{ fontSize: 13, padding: "11px 0", letterSpacing: 0.5 }}>
            {podeIniciar
              ? `▶  Iniciar — ${tipoObj.nome}${tipoObj.subconsultas && subSel ? " · " + tipoObj.subconsultas.find(s => s.id === subSel)?.nome : ""}`
              : "Selecciona o tipo de consulta acima"}
          </button>
        </div>
      )}
    </div>
  ...
  if (!aceite) return (
    <div className="fade">
      <div className="card">
        ...
        <button className="btn btn-p" onClick={() => { registarAceite(user?.id, "metodo"); setAceite(true); }}>
          Li e aceito — sou responsável
        </button>
      </div>
    </div>
  );
...
const FORMS_DEF = [
  ...
  },
  {
    ...
    descricao: "Perguntas dirigidas à criança, em linguagem simples.",
    blocos: [
      { titulo: "Identificação", perguntas: [
        { id: "nome", q: "Qual é o teu nome?", tipo: "texto" },
        { id: "idade", q: "Quantos anos tens?", tipo: "texto" },
      ] },
      { titulo: "O que gostas de fazer", perguntas: [
        { id: "brincadeiras", q: "Quais são as tuas brincadeiras preferidas?", tipo: "texto" },
        { id: "amigos", q: "Quais são os teus amigos?", tipo: "texto" },
        { id: "escola", q: "O que gostas de fazer na escola?", tipo: "texto" },
        { id: "desenhos", q: "Quais são os teus desenhos animados favoritos?", tipo: "texto" },
      ] },
      { titulo: "Como te sentes", perguntas: [
        { id: "sentir", q: "Sentes-te feliz, triste ou com raiva?", tipo: "escolha", opcoes: ["Feliz", "Triste", "Com raiva"] },
        { id: "quando", q: "Quando te sentes assim?", tipo: "texto" },
        { id: "fazes", q: "O que fazes quando te sentes assim?", tipo: "texto" },
      ] },
      { titulo: "O que te preocupa", perguntas: [
        { id: "medo", q: "Há alguma coisa que te deixa preocupado ou com medo?", tipo: "texto" },
        { id: "feliz", q: "O que te deixa feliz?", tipo: "texto" },
        { id: "triste", q: "O que te deixa triste?", tipo: "texto" },
      ] },
      { titulo: "Os teus sonhos", perguntas: [
        { id: "ser", q: "O que queres ser quando fores grande?", tipo: "texto" },
        { id: "mundo", q: "O que gostarias de mudar no mundo?", tipo: "texto" },
      ] },
    ],
  },
  {
    key: "ficha_cuidador",
    titulo: "Ficha do Cuidador (Pais/Educador)",
    descricao: "Anamnese funcional respondida pelo cuidador responsável.",
    blocos: [
      { titulo: "Identificação", perguntas: [
        { id: "nome_crianca", q: "Nome completo da criança", tipo: "texto" },
        { id: "nascimento", q: "Data de nascimento", tipo: "texto" },
        { id: "idade", q: "Idade", tipo: "texto" },
        { id: "quem", q: "Quem está a responder e grau de parentesco", tipo: "texto" },
      ] },
      { titulo: "Queixa principal", perguntas: [
        { id: "motivo", q: "Qual o motivo da consulta?", tipo: "texto" },
        { id: "tempo", q: "Há quanto tempo os sintomas estão presentes?", tipo: "texto" },
        { id: "feito", q: "O que já foi feito até ao momento?", tipo: "texto" },
        { id: "manifesta", q: "Como se manifesta no dia a dia? Quando agrava ou melhora?", tipo: "texto" },
      ] },
      { titulo: "Desenvolvimento", perguntas: [
        { id: "gravidez", q: "Gravidez e parto: houve complicações?", tipo: "texto" },
        { id: "marcos", q: "Atingiu os marcos de desenvolvimento na idade esperada?", tipo: "texto" },
        { id: "linguagem", q: "Linguagem: compreende e faz-se entender?", tipo: "texto" },
        { id: "alimentacao", q: "Hábitos alimentares, alergias ou intolerâncias", tipo: "texto" },
        { id: "sono", q: "Sono: dorme bem? Há dificuldades?", tipo: "texto" },
      ] },
      { titulo: "Família e escola", perguntas: [
        { id: "familia", q: "Relação com a família e conflitos existentes", tipo: "texto" },
        { id: "escola", q: "Frequenta a escola? Desempenho e amigos?", tipo: "texto" },
        { id: "dificuldade", q: "Dificuldades de aprendizagem ou comportamento?", tipo: "texto" },
      ] },
      { titulo: "Hábitos e saúde", perguntas: [
        { id: "habitos", q: "Hábitos específicos (chupar dedo, roer unhas...)?", tipo: "texto" },
        { id: "ecra", q: "Televisão/dispositivos: com que frequência?", tipo: "texto" },
        { id: "doenca", q: "Doença crónica ou medicação?", tipo: "texto" },
        { id: "exames", q: "Já realizou exame psicológico ou psiquiátrico?", tipo: "texto" },
      ] },
    ],
  },
];
...
function Infanto({ adminMode, ir }) {
  ...
  const persistir = async (lista) => {
    ...
    const { error } = await sb.from("config_global").upsert({ chave: "infanto", valor: lista }, { onConflict: "chave" });
  ...
  const editar = (it) => { setNovo({ titulo: it.titulo || "", faixaEtaria: it.faixaEtaria || "", descricao: it.descricao || "", notas: it.notas || "" }); setEditId(it.id); };
  const apagar = (id) => { if (confirm("Apagar esta ficha?")) persistir(itens.filter(i => i.id !== id)); };

  return (
    <div className="fade">
      {ir && (
        <div className="card">
          <div className="card-t">Ações infantis</div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <button className="btn btn-p btn-sm" style={{ width: "auto" }} onClick={() => ir("mapeamento")}>🩺 Nova consulta</button>
            <button className="btn btn-s btn-sm" style={{ width: "auto" }} onClick={() => ir("questionario", "anamnese_crianca")}>📋 Anamnese da Criança</button>
            <button className="btn btn-s btn-sm" style={{ width: "auto" }} onClick={() => ir("questionario", "ficha_cuidador")}>📋 Ficha dos Pais/Educador</button>
          </div>
        </div>
      )}
      <div className="card">
        <div className="card-t">👶 Infanto-Juvenil</div>
        <div className="al al-i" style={{ fontSize: 10 }}>Conteúdos e protocolos para atendimento infanto-juvenil.</div>
        {itens.length === 0 && <div style={{ fontSize: 10, color: "#2d4a66", marginTop: 8 }}>Sem conteúdos ainda.</div>}
        {itens.map(item => (
          <div key={item.id} style={{ marginBottom: 8, paddingBottom: 8, borderBottom: "1px solid #0d1828" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 7 }}>
              <div onClick={() => setAberto(aberto === item.id ? null : item.id)} style={{ fontWeight: 700, fontSize: 12, color: "#00c6b8", cursor: "pointer", flex: 1 }}>{aberto === item.id ? "▾" : "▸"} {item.titulo} {item.faixaEtaria && <span style={{ color: "#2d4a66", fontWeight: 400 }}>· {item.faixaEtaria}</span>}</div>
              {adminMode && (
                <div style={{ display: "flex", gap: 4 }}>
                  <button className="btn btn-s btn-sm" style={{ width: "auto", padding: "2px 7px", fontSize: 10 }} onClick={() => editar(item)}>✏️</button>
                  <button className="btn btn-d btn-sm" style={{ width: "auto", padding: "2px 7px", fontSize: 10 }} onClick={() => apagar(item.id)}>🗑️</button>
                </div>
              )}
            </div>
            {aberto === item.id && <>
      ...
      {adminMode && (
        <div className="card">
          <div className="card-t">{editId ? "Editar ficha" : "Adicionar ficha"}</div>
          <div className="lbl">Título</div>
          <input className="inp" value={novo.titulo} onChange={e => setNovo({ ...novo, titulo: e.target.value })} />
          <div className="lbl">Faixa etária</div>
          <input className="inp" value={novo.faixaEtaria} onChange={e => setNovo({ ...novo, faixaEtaria: e.target.value })} placeholder="ex: 5-7 anos" />
          ...
          <div className="lbl">Notas / sinais de alerta</div>
          <textarea className="inp" rows={3} value={novo.notas} onChange={e => setNovo({ ...novo, notas: e.target.value })} />
          <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
            <button className="btn btn-p btn-sm" style={{ width: "auto" }} onClick={guardarFicha}>{editId ? "Guardar alterações" : "+ Adicionar"}</button>
            {editId && <button className="btn btn-s btn-sm" style={{ width: "auto" }} onClick={() => { setNovo({ titulo: "", faixaEtaria: "", descricao: "", notas: "" }); setEditId(null); }}>Cancelar</button>}
          </div>
        </div>
      )}
    </div>
