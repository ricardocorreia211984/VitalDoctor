...
function SitePreview({ cfg }) {
  ...
  const conteudoCategoria = (id) => {
    ...
    if (id==="servicos") return cfg.servicos.map((s,i)=><ItemCard key={i} it={s} lista="servicos" />);
    if (id==="formacoes") return cfg.formacoes.map((s,i)=><ItemCard key={i} it={s} lista="formacoes" />);
    if (id==="atividades") return cfg.atividades.map((s,i)=><ItemCard key={i} it={s} lista="atividades" />);
    if (id==="produtos") return cfg.produtos.map((s,i)=><ItemCard key={i} it={s} lista="produtos" />);
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {SECS.map(([id,icon,lbl])=>(
            <button key={id} onClick={()=>setAba(id)}
              style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8,padding:"22px 10px",borderRadius:18,border:`1.5px solid ${cor}25`,background:`linear-gradient(160deg,${cor}10,#fff)`,cursor:"pointer",transition:"all .15s",boxShadow:"0 2px 8px rgba(0,0,0,.04)"}}>
