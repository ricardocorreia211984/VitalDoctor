      ...
      <div className="app">
        ...
        <main className="main">
          ...
          <div className="main-body">
            ...
            {mod === "metodo"       && temMod("avancado") && <ModuloMetodo user={perfil} adminMode={isAdmin} />}
...
function Mapeamento({ user }) {
  ...
  const [dados, setDados] = useState({ nome:"", dataNasc:"", dataAval: new Date().toISOString().slice(0,10), medicacao:"" });
  ...
  if (etapa === "dados") return (
    <div className="fade">
      <div className="card">
        ...
        <input className="inp" value={dados.nome} onChange={e=>setDados(d=>({...d,nome:e.target.value}))} placeholder="Nome completo do paciente" />
        ...
        <button className="btn btn-p" style={{marginTop:8}} onClick={()=>setEtapa("mapa")} disabled={!dados.nome.trim()}>Iniciar mapeamento →</button>
      </div>
    </div>
  );
    ...
    <div className="fade">
      ...
      {res && <>
        <div className="card" style={{marginTop:10}}>
          ...
          <button className="btn btn-s btn-sm" style={{width:"auto",marginTop:6}} onClick={()=>navigator.clipboard?.writeText(
            `Relatório — ${dados.nome} · ${dados.dataAval}\nEscudo: ${ESCUDOS.find(e=>e.id===escudo)?.nome}\n` + sel.map(x=>{const p=getPonto(x.id); return `\n${p?.nome} (${x.lado}/${x.face})\n• ${p?.aspectos}\n• Sinais: ${p?.sintomas}\n• Pergunta: ${p?.frase}`;}).join("\n")
...
function ModuloMetodo({ user, adminMode }) {
  ...
  const tabs = [["consulta","🩺 Nova Consulta"],["mapeamento","🗺️ Mapeamento"],["questionario","📋 Questionários"],["assistente","🤖 Assistente"],["farmacia","🌿 Farmácia"],["infanto","👶 Infanto"],["audios","🎧 Áudios"]];
  return (
    <div className="fade">
      <div className="card">
