...
function gerarRelatorioFiel(tipo, dados, pacienteNome) {
  ...
  if (tipo === "Mapeamento Energético Vital") {
    ...
    const { cab, matrizes, lateralidade, lateralidadeTexto, quando, notas } = dados;
    ...
    if (lateralidade || lateralidadeTexto) linhas.push(`\nLATERALIDADE\n  ${lateralidade || ""}${lateralidadeTexto ? ` — Zona detetada: ${lateralidadeTexto}` : ""}`);
...
function FormMapeamentoGrelha({ paciente, user, onGuardar, onVoltar }) {
  // Colunas exactas do formulário original
  ...
  const [lateralidadeTexto, setLateralidadeTexto] = useState("");
  ...
  const handleGuardar = async () => {
    ...
    const dados = { cab, matrizes, lateralidade, lateralidadeTexto, quando, notas };
    ...
    <div className="fade">
      ...
      <div className="card" style={{textAlign:"center",padding:"24px 20px"}}>
        ...
        <div style={{display:"flex",gap:8,marginTop:14}}>
          <button className="btn btn-p" onClick={()=>{setMatrizes({pv:{},pe:{},ss:{},sc:{},si:{},ea:{}});setLateralidade("");setLateralidadeTexto("");setQuando({trans:false,gestacao:false,apos:false,texto:""});setNotas("");setGuardado(false);setRelatorio("");}}>Novo Mapeamento</button>
          <button className="btn btn-s" onClick={onVoltar}>← Voltar</button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fade">
      {/* Cabeçalho — campos exactos do formulário original */}
      <div className="card">
        ...
        <div className="g2">
          <div>
            <div className="lbl">DATA DE NASCIMENTO DO PACIENTE *</div>
            <input className="inp" type="date" value={cab.dataNasc} onChange={e=>setCab(c=>({...c,dataNasc:e.target.value}))} />
          </div>
          <div>
            <div className="lbl">DATA DA AVALIAÇÃO *</div>
            <input className="inp" type="date" value={cab.dataAval} onChange={e=>setCab(c=>({...c,dataAval:e.target.value}))} />
          </div>
        </div>
      ...
      {/* 1. PONTOS VITAIS — linhas e colunas EXACTAS do formulário original */}
      <MatrixSelector titulo="PONTOS VITAIS" 
        linhas={["FRENTE.ESQ.","FRENTE.DRT.","COSTAS.ESQ.","COSTAS.DRT."]}
        colunas={["COROA","GARGANTA","CORAÇÃO","PLEXO (boca/estômago)","PLEXO SOLAR","ESPLÉNICO","RAIZ"]}
        values={matrizes.pv} onChange={(l,c)=>upd("pv",l,c)} />

      {/* 2. PONTOS DE ENTRADA — 14 linhas exactas (Drt + Esq) */}
      <MatrixSelector titulo="PONTOS DE ENTRADA"
        linhas={["COROA.Drt.","OMBRO.Drt.","COSTELAS.Drt.","MÃO.Drt.","COXA.Drt.","JOELHO.Drt.","PÉ.Drt.","COROA.Esq.","OMBRO.Esq.","COSTELAS.Esq.","MÃO.Esq.","COXA.Esq.","JOELHO.Esq.","PÉ.Esq."]}
        colunas={COLUNAS}
        values={matrizes.pe} onChange={(l,c)=>upd("pe",l,c)} />

      {/* 3. LATERALIDADE — escolha + texto livre (exacto ao original) */}
      <div className="card" style={{marginBottom:10}}>
        <div className="card-t">LATERALIDADE</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
          {COLUNAS.map(c=>(
            <button key={c} className={`chip ${lateralidade===c?"on":""}`} onClick={()=>setLateralidade(c)}>{c}</button>
          ))}
        </div>
        <div className="lbl">LATERALIDADE (escreve a zona detetada) *</div>
        <input className="inp" value={lateralidadeTexto} onChange={e=>setLateralidadeTexto(e.target.value)} placeholder="Zona onde o movimento travou..." />
        {lateralidade && <div style={{marginTop:8,padding:"7px 10px",background:"rgba(0,198,184,.05)",borderRadius:6,fontSize:10,color:"#5ae0d8"}}>
          {lateralidade.includes("Esq")?"🔵 Lado esquerdo — trauma racionalizado: ansiedade, stress, burnout (lógica e contenção).":"🔴 Lado direito — trauma emocionalizado: depressão, luto, desamparo (sensibilidade e profundidade)."}
        </div>}
      </div>

      {/* 4. SISTEMA SUPERIOR — 9 pontos exactos */}
      <MatrixSelector titulo="SISTEMA SUPERIOR"
        linhas={["EPÍFISE","HIPOTÁLAMO","HIPÓFISE","AMÍGDALAS","PARATIROIDE","TIMO","GLÂNDULAS SALIVARES E LACRIMAIS","TIREOIDE","ESÓFAGO"]}
        colunas={COLUNAS}
        values={matrizes.ss} onChange={(l,c)=>upd("ss",l,c)} />

      {/* 5. SISTEMA CENTRAL — 12 pontos exactos */}
      <MatrixSelector titulo="SISTEMA CENTRAL"
        linhas={["VASO LINFÁTICO, artérias e veias","INTESTINO GROSSO","CORAÇÃO","BRÔNQUIOS","ALVÉOLOS PULMONARES","INTESTINO DELGADO","BAÇO","FÍGADO","ESTÔMAGO","DUODENO","VESÍCULA BILIAR","PÂNCREAS"]}
        colunas={COLUNAS}
        values={matrizes.sc} onChange={(l,c)=>upd("sc",l,c)} />

      {/* 6. SISTEMA INFERIOR — 6 pontos exactos */}
      <MatrixSelector titulo="SISTEMA INFERIOR"
        linhas={["GLÂNDULAS MAMÁRIAS","ÚTERO, PRÓSTATA","SUPRARRENAIS","TESTÍCULOS, OVÁRIOS","RINS","BEXIGA"]}
        colunas={COLUNAS}
        values={matrizes.si} onChange={(l,c)=>upd("si",l,c)} />

      {/* 7. ESCUDO MAIS ATIVO */}
      <MatrixSelector titulo="ESCUDO MAIS ATIVO"
        linhas={["DESPROTEÇÃO","DESVALORIZAÇÃO","SOBREVIVÊNCIA","PERDA","IMPOTÊNCIA"]}
        colunas={COLUNAS}
        values={matrizes.ea} onChange={(l,c)=>upd("ea",l,c)} />

      {/* 8. QUANDO — checkboxes + idade obrigatória (exacto ao original) */}
      <div className="card">
      ...
      {/* Devolutiva e notas do terapeuta */}
      <div className="card">
        <div className="card-t">DEVOLUTIVA E NOTAS DA SESSÃO</div>
        <div style={{fontSize:10,color:"#5a7a9a",marginBottom:6}}>Entrega ao paciente o que o corpo revelou, com presença e sensibilidade. Indica o protocolo de áudio e os pontos a trabalhar por 7 ou 15 dias.</div>
        <textarea className="inp" rows={4} value={notas} onChange={e=>setNotas(e.target.value)} placeholder="Correlação ponto → escudo → história emocional · Áudio recomendado · Pontos a trabalhar · Próximos passos..." />
      </div>
