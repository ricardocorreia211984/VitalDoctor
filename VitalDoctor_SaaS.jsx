...
function gerarRelatorioFiel(tipo, dados, pacienteNome) {
  ...
  if (tipo === "Mapeamento Energético Vital") {
    linhas.push(`\nMAPEAMENTO ENERGÉTICO VITAL`);
    const { matrizes, lateralidade, quando, notas } = dados;
    const imprimirMatrix = (label, obj) => {
      const entradas = Object.entries(obj||{}).filter(([,v])=>v);
      if (!entradas.length) return;
      linhas.push(`\n${label}`);
      entradas.forEach(([linha,col])=>linhas.push(`  ${linha}: ${col}`));
    };
    imprimirMatrix("PONTOS VITAIS", matrizes?.pv);
    imprimirMatrix("PONTOS DE ENTRADA", matrizes?.pe);
    if (lateralidade) linhas.push(`\nLATERALIDADE\n  ${lateralidade}`);
    imprimirMatrix("SISTEMA SUPERIOR", matrizes?.ss);
    imprimirMatrix("SISTEMA CENTRAL",  matrizes?.sc);
    imprimirMatrix("SISTEMA INFERIOR",  matrizes?.si);
    imprimirMatrix("ESCUDO ATIVO",      matrizes?.ea);
    const q = [];
    if (quando?.trans)   q.push("Transgeracional");
    if (quando?.gestacao) q.push("Gestacional");
    if (quando?.apos)    q.push("Pós-parto");
    if (quando?.texto)   q.push(quando.texto);
    if (q.length) linhas.push(`\nTEMPO DO CONFLITO\n  ${q.join(" · ")}`);
  ...
  linhas.push(`\n${"─".repeat(55)}`);
  linhas.push(`⚠️ Ferramenta de apoio — não substitui diagnóstico médico.`);
...
function FormMapeamentoGrelha({ paciente, user, onGuardar, onVoltar }) {
  const COLUNAS = ["FRENTE.Drt.", "FRENTE.Esq.", "COSTAS.Esq.", "COSTAS.Drt."];
  const [matrizes, setMatrizes] = useState({ pv:{}, pe:{}, ss:{}, sc:{}, si:{}, ea:{} });
  ...
  const upd = (m,l,c) => setMatrizes(p=>({...p,[m]:{...p[m],[l]:c}}));

  const handleGuardar = async () => {
    const dados = { matrizes, lateralidade, quando, notas };
  ...
  if (guardado) return (
    <div className="fade">
      ...
      <div className="card" style={{textAlign:"center",padding:"24px 20px"}}>
        ...
        <div style={{fontSize:14,fontWeight:700,color:"#b0c4d8",marginBottom:4}}>Mapeamento guardado na ficha!</div>
        <div style={{display:"flex",gap:8,marginTop:14}}>
          <button className="btn btn-p" onClick={()=>{setMatrizes({pv:{},pe:{},ss:{},sc:{},si:{},ea:{}});setLateralidade("");setQuando({trans:false,gestacao:false,apos:false,texto:""});setNotas("");setGuardado(false);setRelatorio("");}}>Novo Mapeamento</button>
          <button className="btn btn-s" onClick={onVoltar}>← Voltar</button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fade">
      <div className="card">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <button className="btn btn-s btn-sm" style={{width:"auto",marginRight:8}} onClick={onVoltar}>←</button>
            <strong style={{color:"#dde4f0",fontSize:13}}>Mapeamento Energético Vital</strong>
          </div>
          <div style={{fontSize:10,color:"#00c6b8"}}>{paciente?.nome||""}</div>
        </div>
        <div style={{fontSize:10,color:"#5a7a9a",marginTop:6,lineHeight:1.6}}>
          Preenche cada mapa clicando no quadrante correspondente. Segue a ordem dos passos.
        </div>
      </div>

      {/* Mapa 1: Pontos Vitais */}
      <MatrixSelector titulo="PASSO 1 — ENERGIA VITAL (7 Centros)" linhas={["Coronário","Laríngeo","Cardíaco","Plexo Boca/Estômago","Plexo Solar","Esplénico"]} colunas={COLUNAS} values={matrizes.pv} onChange={(l,c)=>upd("pv",l,c)} />

      {/* Mapa 2: Pontos de Entrada (Zona de Impacto) */}
      <MatrixSelector titulo="PASSO 2 — ZONA DE IMPACTO (13 Pontos)" linhas={["Topo da Cabeça","Ombro Drt.","Ombro Esq.","Costelas Drt.","Costelas Esq.","Mão Drt.","Mão Esq.","Anca/Quadril","Joelho Drt.","Joelho Esq.","Pé Drt.","Pé Esq."]} colunas={COLUNAS} values={matrizes.pe} onChange={(l,c)=>upd("pe",l,c)} />

      {/* Mapa 3: Lateralidade */}
      <div className="card">
        <div className="card-t">PASSO 3 — LATERALIDADE (Superfície)</div>
        <div style={{fontSize:10,color:"#5a7a9a",marginBottom:10,lineHeight:1.6}}>
          Desliza ao redor do corpo — para onde a mão travar é a lateralidade do trauma.<br/>
          <span style={{color:"#b0c4d8"}}>Esq. = Trauma racionalizado (Ansiedade/Burnout) · Drt. = Trauma emocionalizado (Depressão/Luto)</span>
        </div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {[...COLUNAS,"Lateral Esq.","Lateral Drt."].map(c=>(
            <button key={c} className={`chip ${lateralidade===c?"on":""}`} onClick={()=>setLateralidade(c)}>{c}</button>
          ))}
        </div>
        {lateralidade && <div style={{marginTop:8,padding:"7px 10px",background:"rgba(0,198,184,.05)",borderRadius:6,fontSize:10,color:"#5ae0d8"}}>
          {lateralidade.includes("Esq")||lateralidade.includes("ESQ")?"🔵 Lado esquerdo — Trauma racionalizado: ansiedade, stress, burnout. Abordagem lógica e analítica.":"🔴 Lado direito — Trauma emocionalizado: depressão, luto, desamparo. Resposta sensível e profunda."}
        </div>}
      </div>

      {/* Mapa 4: Sistemas */}
      <MatrixSelector titulo="PASSO 4A — SISTEMA SUPERIOR (Conexão com o Ser)" linhas={["Epífise (Pineal)","Hipotálamo","Hipófise (Pituitária)","Amígdalas","Paratireoide","Timo","Glândulas Salivares/Lacrimais","Tireoide","Esófago"]} colunas={COLUNAS} values={matrizes.ss} onChange={(l,c)=>upd("ss",l,c)} />
      <MatrixSelector titulo="PASSO 4B — SISTEMA CENTRAL (Corpo Emocional)" linhas={["Vasos Linfáticos/Artérias/Veias","Intestino Grosso","Coração","Brônquios","Alvéolos Pulmonares","Intestino Delgado","Baço","Fígado","Estômago","Duodeno","Vesícula Biliar","Pâncreas"]} colunas={COLUNAS} values={matrizes.sc} onChange={(l,c)=>upd("sc",l,c)} />
      <MatrixSelector titulo="PASSO 4C — SISTEMA INFERIOR (Raízes e Pertencimento)" linhas={["Glândulas Mamárias","Útero / Próstata","Suprarrenais","Testículos / Ovários","Rins","Bexiga"]} colunas={COLUNAS} values={matrizes.si} onChange={(l,c)=>upd("si",l,c)} />

      {/* Passo 5: Escudo Ativo */}
      <MatrixSelector titulo="PASSO 5 — ESCUDO ATIVO (Conflito Predominante)" linhas={["Desproteção","Desvalorização","Impotência","Sobrevivência","Perda"]} colunas={COLUNAS} values={matrizes.ea} onChange={(l,c)=>upd("ea",l,c)} />

      {/* Passo 6+7: Tempo do conflito */}
      <div className="card">
        <div className="card-t">PASSO 6+7 — TEMPO DO CONFLITO</div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
          {[["trans","Transgeracional"],["gestacao","Gestacional"],["apos","Pós-parto"]].map(([k,l])=>(
            <button key={k} className={`chip ${quando[k]?"on":""}`} onClick={()=>setQuando(q=>({...q,[k]:!q[k]}))}>{l}</button>
          ))}
        </div>
        <span className="lbl">Especificar (geração, mês, idade)</span>
        <input className="inp" value={quando.texto} onChange={e=>setQuando(q=>({...q,texto:e.target.value}))} placeholder="Ex: 2 gerações · Gestacional 3º mês · Infância, 7 anos..." />
      </div>

      {/* Passo 8: Devolutiva */}
      <div className="card">
        <div className="card-t">PASSO 8 — DEVOLUTIVA E NOTAS</div>
        <div style={{fontSize:10,color:"#5a7a9a",marginBottom:6}}>Entrega ao paciente o que o corpo revelou, com presença e sensibilidade.</div>
        <textarea className="inp" rows={4} value={notas} onChange={e=>setNotas(e.target.value)} placeholder="Correlação ponto → escudo → sintoma → história emocional · Protocolo de áudio recomendado · Próximos passos..." />
      </div>

      <button className="btn btn-p" style={{padding:"12px 0",fontSize:13}} onClick={handleGuardar} disabled={load}>
...
// ══════════════════════════════════════════════════════════════════
// FORMULÁRIO C — ATENDIMENTO ESTRUTURADO (3 Caminhos)
// Para Consultas de Seguimento, Pack 1, Pack 3
// ══════════════════════════════════════════════════════════════════
  ...
  const handleGuardar = async (tipo, dados) => {
    try {
      ...
      await sb.from("consultas").insert({
        ...
        terapeuta_id: user.id,        // ISOLAMENTO: cada consulta vinculada ao seu terapeuta
        tipo,
