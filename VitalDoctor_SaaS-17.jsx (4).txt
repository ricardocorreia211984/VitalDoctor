...
function Dashboard({ user, pacs, agenda, go }) {
  ...
  const [dash, setDash] = useState(DASH_PADRAO);
  ...
  return (
    <div className="fade">
      ...
      {/* STATS */}
      <div className="stats">
        <div className="stat"><div className="stat-n">{pacs.length||<span style={{color:"#3d5a7a"}}>—</span>}</div><div className="stat-l">Pacientes</div></div>
        <div className="stat"><div className="stat-n">{hojeMs.length||<span style={{color:"#3d5a7a"}}>—</span>}</div><div className="stat-l">Hoje</div></div>
        <div className="stat"><div className="stat-n">€{totalPago.toFixed(0)}</div><div className="stat-l">Recebido</div></div>
      ...
      {/* AGENDA HOJE */}
      <div className="card" style={{marginBottom:10}}>
        <div className="card-t">📅 Agenda de Hoje</div>
        {hojeMs.length === 0
      ...
      {modal && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:"#0a0e18",border:"1px solid #1a3a5c",borderRadius:12,width:"100%",maxWidth:420,maxHeight:"90vh",overflow:"auto",padding:18}}>
            ...
            <div className="g2">
              <div><span className="lbl">Tipo</span><select className="inp sel" value={nova.tipo} onChange={e => setNova({...nova,tipo:e.target.value})}><option>Consulta</option><option>Avaliação Energética</option><option>Seguimento</option><option>Pack</option></select></div>
...
function SeletorPacienteUniversal({ user, titulo, onSelecionado, onVoltar }) {
  ...
  useEffect(() => {
    sb.from("pacientes").select("id,nome,telefone,email,data_nascimento")
      ...
      <div style={{maxHeight:360,overflowY:"auto"}}>
        {filtrados.map(p => (
          <div key={p.id} onClick={() => onSelecionado(p)}
            style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:"#050810",border:"1px solid #0d1828",borderRadius:8,marginBottom:5,cursor:"pointer",transition:"all .15s"}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor="#00c6b8";}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor="#0d1828";}}>
            <div style={{width:34,height:34,borderRadius:"50%",background:"linear-gradient(135deg,#1a4a6c,#0d2535)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0}}>👤</div>
            <div style={{flex:1}}>
              <div style={{fontWeight:700,fontSize:12,color:"#b0c4d8"}}>{p.nome}</div>
              <div style={{fontSize:9,color:"#3d5a7a"}}>{p.telefone||p.email||"Sem contacto"} {p.data_nascimento&&"· "+new Date(p.data_nascimento).toLocaleDateString("pt-PT")}</div>
            </div>
...
function PainelDistancia({ user }) {
      ...
      onGuardar={async(tipo,dados)=>{
        ...
        if (sel.id) {
          ...
          await sb.from("consultas").insert({
            ...
            tipo:"Trabalho à Distância — "+( dados.nome_completo||""),
            dados_formulario:dados, data:new Date().toISOString().split("T")[0]
          });
        }
        setMsg("✅ Guardado na ficha do paciente!");setTimeout(()=>setMsg(""),2500);
        setSel(null); setPacSel(null); setFase("lista"); reload();
      }}
  ...
  if (fase === "selecionar_pac") return (
    <SeletorPacienteUniversal user={user} titulo="🔯 Trabalho à Distância — Escolher Paciente"
    ...
    <div className="fade">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:12}}>
        ...
        <div onClick={()=>setSub("distancia")}
          ...
          <div style={{fontSize:20,marginBottom:4}}>🔯</div>
          <div style={{fontSize:11,fontWeight:800,color:sub==="distancia"?"#c8a8f0":"#8ba3c0"}}>À Distância</div>
          <div style={{fontSize:9,color:"#5a3a7a",marginTop:2}}>HON SHA ZE SHO NEN · Qualquer caso</div>
        </div>
      </div>
        ...
        <div className="card">
          <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
            ...
            <div style={{fontWeight:700,color:"#dde4f0",fontSize:13}}>
              {tituloConsulta || (formAtivo==="form_a"?"🩺 1º Atendimento":formAtivo==="form_b"?"🗺️ Avaliação Energética Vital":formAtivo==="custom_module"?"📋 Módulo Personalizado":"🧭 Atendimento Estruturado")}
            </div>
          </div>
...
function Assistente({ user }) {
  ...
  useEffect(() => { _carregarMetodoConteudo(); }, []);
  ...
  const analisar = () => {
    ...
    const scorePt = pontosComOverride().map(p => {
    ...
    setSug({ escudos:scoreEsc, pontos:scorePt, afirm, perguntas:getConteudoMetodo("perguntas_abertura", PERGUNTAS_ABERTURA).slice(0,3), semMatch: scoreEsc.length===0 && scorePt.length===0 });
  };
