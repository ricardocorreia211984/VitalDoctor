...
import { PONTOS_POR_SISTEMA, getPonto } from "./mapaCorporal.js";
import { ESCUDOS, QUESTIONARIO_ESCUDOS, ESCALA_QUESTIONARIO } from "./baseConhecimento.js";
import { MAPEAMENTO_PASSOS, TIPOS_ATENDIMENTO, passosDoTipo } from "./atendimento.js";
import { gerarProtocoloCura } from "./protocoloCura.js";
      ...
      <div className="app">
        ...
        <main className="main">
          ...
          <div className="main-body">
            ...
            {mod === "metodo"       && temMod("avancado") && <ModuloMetodo user={perfil} />}
...
function Mapeamento({ user }) {
  ...
  const sugerir = () => {
    const c = {}; sel.forEach(x => (getPonto(x.id)?.escudos || []).forEach(e => c[e] = (c[e] || 0) + 1));
    const t = Object.entries(c).sort((a, b) => b[1] - a[1])[0]; if (t) setEscudo(t[0]);
  };
  const gerar = () => setRes(gerarProtocoloCura({ paciente: { nome: "" }, sexo, escudo, protocoloDias: dias, modo, mapeamento: sel.map(x => ({ id: x.id, lado: x.lado, face: x.face })) }));

  const Sis = ({ titulo, lista }) => (
    <div style={{ marginBottom: 10 }}>
      <div className="lbl" style={{ marginBottom: 4 }}>{titulo}</div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
        {lista.map(p => <button key={p.id} className="chip" onClick={() => add(p)}>{p.nome} +</button>)}
      </div>
    </div>
  );
  ...
  return (
    <div className="fade">
      <div className="card">
        <div className="card-t">Mapeamento Energético</div>
        <button className="btn btn-s btn-sm" style={{ width: "auto" }} onClick={() => setGuia(g => !g)}>{guia ? "Ocultar guia" : "Ver guia dos 4 mapas"}</button>
        {guia && <div style={{ marginTop: 8, fontSize: 11, color: "#5a7a9a", lineHeight: 1.5 }}>
          {MAPEAMENTO_PASSOS.map(p => <div key={p.n} style={{ marginBottom: 5 }}><strong>{p.titulo}:</strong> {p.texto}</div>)}
        </div>}
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
          {["frente", "costas"].map(f => <button key={f} className={`chip ${face === f ? "on" : ""}`} onClick={() => setFace(f)}>{f}</button>)}
        </div>
        <div className="card-t">Sistemas — toca para adicionar o ponto</div>
        <Sis titulo="Superior" lista={PONTOS_POR_SISTEMA.Superior} />
        <Sis titulo="Central" lista={PONTOS_POR_SISTEMA.Central} />
        <Sis titulo="Inferior" lista={PONTOS_POR_SISTEMA.Inferior} />
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 6 }}>
          {ESCUDOS.map(e => <button key={e.id} className={`chip ${escudo === e.id ? "on" : ""}`} onClick={() => setEscudo(e.id)}>{e.nome}</button>)}
        </div>
      ...
      <button className="btn btn-p" onClick={gerar} disabled={!sel.length || !escudo}>Gerar protocolo de cura</button>
      ...
      {res && <div className="card" style={{ marginTop: 10 }}>
        ...
        <button className="btn btn-s btn-sm" style={{ width: "auto", marginTop: 8 }} onClick={() => navigator.clipboard?.writeText(res.texto)}>Copiar</button>
      </div>}
    </div>
  );
}
...
function Questionario() {
  ...
  const [res, setRes] = useState(null);
  const set = (b, i, v) => setResp(r => ({ ...r, [b]: { ...(r[b] || {}), [i]: v } }));
  const calcular = () => {
    const q = {};
    QUESTIONARIO_ESCUDOS.forEach(b => { q[b.blocoId] = b.afirmacoes.map((_, i) => (resp[b.blocoId]?.[i]) || 0); });
    setRes(pontuarEscudos(q));
  };
  return (
    <div className="fade">
      <div className="card"><div className="card-t">Questionário dos Escudos</div>
        <div className="al al-i" style={{ fontSize: 10 }}>Para cada afirmação: 1 = pouco · 2 = às vezes · 3 = muito.</div>
      </div>
      {QUESTIONARIO_ESCUDOS.map(b => (
        <div key={b.blocoId} className="card">
          <div className="card-t">{b.titulo}</div>
          {b.afirmacoes.map((a, i) => (
            <div key={i} style={{ marginBottom: 7 }}>
              <div style={{ fontSize: 11, color: "#b0c4d8", marginBottom: 3 }}>{i + 1}. {a}</div>
              <div style={{ display: "flex", gap: 5 }}>
                {ESCALA_QUESTIONARIO.map(e => <button key={e.valor} className={`chip ${(resp[b.blocoId]?.[i]) === e.valor ? "on" : ""}`} onClick={() => set(b.blocoId, i, e.valor)}>{e.valor}</button>)}
              </div>
            </div>
          ))}
        </div>
      ))}
      <button className="btn btn-p" onClick={calcular}>Calcular escudo dominante</button>
      {res && (
        <div className="card" style={{ marginTop: 10 }}>
          <div className="card-t">Resultado</div>
          {res.ordenado.map(o => { const e = ESCUDOS.find(x => x.id === o.escudoId); return (
            <div key={o.escudoId} className="admin-row"><span style={{ fontSize: 11, color: o.escudoId === res.dominante ? "#00c6b8" : "#5a7a9a" }}>{e?.nome}</span><strong style={{ color: "#b0c4d8" }}>{o.total}</strong></div>
          ); })}
          {res.dominante && <div className="al al-ok" style={{ marginTop: 8 }}>Escudo dominante: {ESCUDOS.find(x => x.id === res.dominante)?.nome}</div>}
        </div>
      )}
    </div>
  );
...
function NovaConsulta({ user }) {
  ...
  return (
    <div className="fade">
      <div className="card">
        <div className="card-t">Nova Consulta</div>
...
function ModuloMetodo({ user }) {
  ...
  const tabs = [["consulta", "🩺 Nova Consulta"], ["mapeamento", "🗺️ Mapeamento"], ["questionario", "📋 Questionários"], ["farmacia", "🌿 Farmácia"], ["infanto", "👶 Infanto"], ["audios", "🎧 Áudios"]];
  return (
    <div className="fade">
      <div className="card">
      ...
      {aba === "farmacia" && <Farmacia adminMode={false} />}
      {aba === "infanto" && <Infanto />}
