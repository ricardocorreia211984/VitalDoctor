...
export default function VitalDoctor() {
  ...
  const SB_NAV = [
    ...
    { t:"Consultas",  items:[
      { id:"metodo",         icon:"🌿",l:"Centro Terapêutico" },
...
function ModuloExclusivoTab({ user, temAcesso, adminMode, onFarmacia, onInfanto }) {
  ...
  return (
    <div className="fade">
      ...
      {subAba === "farmacia"     && <Farmacia user={user} />}
      {subAba === "assistente"   && <Assistente user={user} />}
      {subAba === "infanto"      && <SecaoInfanto user={user} />}
...
function ModuloMetodo({ user, adminMode, temAcesso, temHikari, initAba, voltar }) {
  ...
  // Módulo Universal — organizado por secções
  ...
  // Módulos exclusivos (só aparecem se activos)
  const TABS_EXCLUSIVOS = [
    ...(temHikari ? [["hikari_tab",  "🔮 Hikari"]] : []),
    ...(temAcesso  ? [["exclusivo",  "🧠 A&D"]]   : []),
  ];
  const tabs = [...TABS_UNIVERSAL, ...TABS_EXCLUSIVOS];
