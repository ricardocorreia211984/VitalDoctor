...
// Funções de aceite por módulo (responsabilidade.js inline)
const jaAceitou = (uid, mod = "saude") => {
  try { return localStorage.getItem("vd_aceite_" + mod + "_" + uid) === "1"; } catch { return false; }
};
const registarAceite = (uid, mod = "saude") => {
  try { localStorage.setItem("vd_aceite_" + mod + "_" + uid, "1"); } catch {}
};
const AVISO_SAUDE = {
  titulo: "Aviso de Saúde",
  texto: "Este módulo é uma ferramenta de apoio ao atendimento terapêutico. Não substitui diagnóstico médico nem tratamento clínico convencional. O terapeuta é o único responsável pelas orientações dadas ao paciente."
};
