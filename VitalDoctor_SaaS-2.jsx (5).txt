    ...
    <div className="fade">
      ...
      <div className="card">
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
          {[["🩺","Consultas","metodo","mapeamento"],["📖","Tipos de Consulta","metodo","consulta"],["👥","Pacientes","pacientes",null],["📅","Agenda","agenda",null],["📋","Questionarios","metodo","questionario"],["🌿","Farmacia","metodo","farmacia"],["🌐","Mini Site","minisite",null]].map(([ic,lb,m,ab]) => (
...
function Mapeamento({ user }) {
  ...
  const guardarGuia = async () => {
    ...
    const pq = passosAtuais.find(p => p.id === "perguntas")?.dados || [];
    const resps = pq.map((q,qi) => notas["q"+qi] ? `• ${q}\n  ${notas["q"+qi]}` : null).filter(Boolean).join("\n");
    const cam = caminhoSel ? `\n\nCaminho escolhido: ${caminhoSel}` : "";
    const resumo = `${tipoObj?.nome||""}${subObj?` · ${subObj.nome}`:""}\n\nPassos:\n` + passosAtuais.map((p,i)=>`${i+1}. ${p.titulo}`).join("\n") + (resps?`\n\nRespostas:\n${resps}`:"") + cam;
    try {
    ...
    <div className="fade">
      <div className="card">
        ...
        {TIPOS_ATENDIMENTO.map(x => (
          <div key={x.id} className="admin-section" style={{cursor:"pointer",borderColor: tipoSel===x.id ? "#00c6b8" : undefined, marginTop:6}} onClick={()=>{setTipoSel(x.id);setSubSel(null);}}>
      ...
      {tipoObj?.subconsultas && (
        <div className="card">
          ...
          {tipoObj.subconsultas.map(sc => (
            <div key={sc.id} className="admin-section" style={{cursor:"pointer",borderColor: subSel===sc.id ? "#00c6b8" : undefined, marginTop:6}} onClick={()=>setSubSel(sc.id)}>
  ...
  if (etapa === "passos") return (
    <div className="fade">
      <div className="card">
        <button className="btn btn-s btn-sm" style={{width:"auto"}} onClick={()=>setEtapa("dados")}>← Dados</button>
        <div className="card-t" style={{marginTop:8}}>{tipoObj?.nome}{subObj ? ` · ${subObj.nome}` : ""} — {dados.nome || "paciente"}</div>
        <div className="al al-i" style={{fontSize:10}}>Segue os passos com o paciente. Este tipo não usa o mapeamento do corpo.</div>
      </div>

      {passosAtuais.map((p, i) => (
        <div key={p.id} className="card">
          <div className="card-t">{i+1}. {p.titulo}</div>
          {p.descricao && <div style={{fontSize:11,color:"#5a7a9a",marginBottom:6}}>{p.descricao}</div>}

          {p.id === "perguntas" && (p.dados||[]).map((q, qi) => (
            <div key={qi} style={{marginBottom:8}}>
              <div className="lbl">{qi+1}. {q}</div>
              <textarea className="inp" rows={2} style={{resize:"vertical"}} value={notas["q"+qi]||""} onChange={e=>setNotas(n=>({...n,["q"+qi]:e.target.value}))} placeholder="Notas da resposta..." />
            </div>
          ))}

          {p.id === "caminho" && (p.dados||[]).map(c => (
            <div key={c.id} className="admin-section" style={{cursor:"pointer",borderColor: caminhoSel===c.id ? "#00c6b8" : undefined, marginTop:6}} onClick={()=>setCaminhoSel(c.id)}>
              <div style={{fontWeight:600,fontSize:11,color:"#b0c4d8"}}>{c.nome}</div>
              {c.indicado && <div style={{fontSize:9,color:"#5a7a9a",marginTop:2}}>{c.indicado}</div>}
              <ul style={{margin:"4px 0 0 16px",padding:0,fontSize:10,color:"#5a7a9a"}}>
                {(c.passos||[]).map((s,si)=><li key={si} style={{marginBottom:2}}>{s}</li>)}
              </ul>
              {c.semMapeamento === false && caminhoSel===c.id && <div style={{fontSize:10,color:"#f59e0b",marginTop:4}}>Este caminho usa mapeamento — para mapear, escolhe o tipo "Mapeamento Energético".</div>}
            </div>
          ))}
        </div>
      ))}
      ...
      <div className="card">
        <button className="btn btn-p" onClick={guardarGuia} disabled={!dados.nome.trim()}>Guardar consulta na ficha</button>
      </div>
    </div>
  );
