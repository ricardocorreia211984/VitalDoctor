...
import { PONTOS_POR_SISTEMA, getPonto, CENTROS_VITAIS, PONTOS_ENTRADA } from "./mapaCorporal.js";
  ...
  return (
    <div className="fade">
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>{["frente", "costas"].map(f => <button key={f} className={`chip ${face === f ? "on" : ""}`} onClick={() => setFace(f)}>{f}</button>)}</div>

        <div className="card-t">1. Centros vitais (Mapa 1)</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 10 }}>{CENTROS_VITAIS.map(c => <button key={c.id} className={`chip ${vitais.includes(c.id) ? "on" : ""}`} onClick={() => tog(vitais, setVitais, c.id)}>{c.nome}</button>)}</div>

        <div className="card-t">2. Pontos de entrada (Mapa 2)</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 10 }}>{PONTOS_ENTRADA.flatMap(z => z.bilateral ? [z.nome + " Dir", z.nome + " Esq"] : [z.nome]).map(o => <button key={o} className={`chip ${entrada.includes(o) ? "on" : ""}`} onClick={() => tog(entrada, setEntrada, o)}>{o}</button>)}</div>

        <div className="card-t">3. Lateralidade (Mapa 3)</div>
        <input className="inp" placeholder="Escreve o local exato detetado" value={lateral} onChange={e => setLateral(e.target.value)} />
      </div>

      <div className="card">
        <div className="card-t">4. Sistemas — toca para adicionar (face: {face})</div>
